...
class LinkedList:
    ...
    def append(self, item: object) -> None:
        ...
        if len(self) is 0:  # Special case: LinkedList is empty.
            ...
            self._get_node(len(self) - 1).next = _Node(item)
    ...
    def __setitem__(self, index: int, item: object) -> None:
        ...
        if index is len(self):
            ...
            if len(self) is 0: # Special case: LinkedList is empty.
                ...
                self._get_node(index - 1).next = _Node(item)
        elif index is 0:
            # We are setting the initial item
            # Safe navigation, self._first can be None, or self._first.next can
            # exist.
            next_node = self._first and self._first.next
            self._first = _Node(item)
            self._first.next = next_node
        else:
    ...
    def extend(self, items: list) -> None:
        ...
        '[1 -> 2 -> 3 -> 4 -> 5 -> 6 -> 7]'
        """
        tail = LinkedList(items)
        end = self._get_node(len(self) - 1)
        assert end.next is None
        end.next = tail._first
    ...
    def __iter__(self) -> Iterator['LinkedList']:
        ...
        """
        iter_node = self._first
        while iter_node is not None:
            yield iter_node.item
    ...
    def count(self, item: object) -> int:
        """
        >>> lst = LinkedList([1, 1, 2])
        >>> lst.count(1)
        2
        """
        return self.fold(lambda sum,
                                value: sum + (1 if value == item else 0), 0)

    def fold(self, f: Callable[['object', 'object'], any],
             initval: object) -> object:
        """Implementation of fold on linked list.
        >>> lst = LinkedList([1, 2, 3])
        >>> func = lambda sum, value : sum + value
        >>> lst.fold(func, 0)
        6
        """
        if initval is None and len(self) == 0:
            raise IndexError
