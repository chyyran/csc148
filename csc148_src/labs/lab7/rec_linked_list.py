...
class LinkedListRec:
    ...
    def __contains__(self, item: object) -> bool:
        ...
        False
        """
        return item == self._first or (self._rest is not None and
                                       item in self._rest)
    ...
    def count(self, item: object) -> int:
        ...
        if self._first is None:
            return 0
        return (1 if item == self._first else 0) + self._rest.count(item)
    ...
    def insert_first(self, item: object) -> None:
        ...
        '1 -> 2 -> 3'
        """
        new_rest = LinkedListRec([self._first])
    ...
    def map(self, f: Callable[[object], object]) -> 'LinkedListRec':
        ...
        '5 -> 7'
        """
        first = LinkedListRec([f(self._first)])
        rest = self._rest and self._rest.map(f)
        first._rest = rest
        return first
