...
def timsort(lst: list) -> None:
    ...
    while len(runs) > 1:
        ...
        _merge(lst, run1[0], run2[0], run2[1])
...
def find_runs2(lst: list) -> List[Tuple[int, int]]:
    ...
    run_start = 0
    run_end = 1
    state = 0  # 0 = Init 1 = Ascending 2 = Descending 3 = Descending Swap 4 = End
    while run_end <= len(lst):
        if run_end >= len(lst):
            # This is the final loop of the state machine.
        ...
        if state == 1:
            ...
            # We don't know if we are ascending or descending yet...
            assert run_end < len(lst), run_end
            if lst[run_end - 1] >= lst[run_end]:
                state = 2
            else:
                state = 1
            ...
            rev_lst = [*lst[0:run_start], *lst[run_start:run_end][::-1],
                       *lst[run_end:]]
...
def find_runs3(lst: list) -> List[Tuple[int, int]]:
    ...
    run_start = 0
    run_end = 1
    state = 0  # 0 = Init 1 = Ascending 2 = Descending 3 = Descending Swap 4 = End 5 = sorting
    while run_end <= len(lst):
        if run_end >= len(lst):
            # This is the final loop of the state machine.
            if state == 0 or state == 1:
        ...
        if state == 5:
            ...
            # We have to reverse the list in place
            if run_end - run_start < MIN_RUN and run_end < len(lst):
                state = 5
                continue
            rev_lst = [*lst[0:run_start], *lst[run_start:run_end][::-1],
                       *lst[run_end:]]
