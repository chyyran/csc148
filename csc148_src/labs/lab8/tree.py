...
class Tree:
    ...
    def branching_factor(self) -> float:
        ...
        if self.is_empty():
            return 0.0
        branch = self._branching_factor_helper()
    ...
    def _branching_factor_helper(self) -> Tuple[int, int]:
        ...
        non_leaf_nodes = sum([i._branching_factor_helper()[1] for i in self._subtrees], 1)
    ...
    def insert(self, item: object) -> None:
        ...
        if choice == 3:
            self._subtrees.append(Tree(item, []))
            return
        subtree = self._subtrees[random.randint(0, len(self._subtrees) - 1)]
        subtree.insert(item)
