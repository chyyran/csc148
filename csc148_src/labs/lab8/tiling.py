...
class Domino:
    ...
    def __str__(self):
        return f"{(self.position[0][0], self.position[0][1]), (self.position[1][0], self.position[1][1])}"
...
def tile_with_dominoes(n: int) -> List['Domino']:
    ...
    **IMPORTANT!!!**
    ...
    if n == 1:
        ...
        upper_left_tiling = [t for t in tile_with_dominoes(n-1)]
        upper_right_tiling = [t for t in tile_with_dominoes(n-1)]
        lower_left_tiling = [t for t in tile_with_dominoes(n-1)]
        lower_right_tiling = [t for t in tile_with_dominoes(n-1)]
        ...
        return upper_right_tiling + upper_left_tiling \
               + lower_right_tiling + lower_left_tiling
...
def _tile_2_by_2() -> List['Domino']:
    ...
    """
    # Remember that the positions here do *not* depend on SQUARE_SIZE.
    v_tile_1 = Domino((0, 0), (0, 1))
    v_tile_2 = Domino((1, 0), (1, 1))

    h_tile_1 = Domino((0, 0), (1, 0))
    h_tile_2 = Domino((1, 1), (0, 1))
    return [v_tile_1, v_tile_2] if bool(random.getrandbits(1)) else [h_tile_1, h_tile_2]
