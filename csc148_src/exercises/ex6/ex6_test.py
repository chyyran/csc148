...
@given(lists(integers(min_value=-10000, max_value=10000),
             min_size=10, max_size=60))
def test_bst_levels_correct(items):
    """Test num_less_than on a larger BST."""
    bst = _list_to_bst(items)
    bst_levels = bst.levels()
    for h in range(bst.height()):
        assert bst.items_at_depth(bst_levels[h][0]) == bst_levels[h][1]
        assert bst.items_at_depth(bst_levels[h][0]) == sorted(bst.items_at_depth(bst_levels[h][0]))

@given(bsts())
def test_items_at_depth_sorted(bst):
    """Test items_at_depth for the root level (d=1)."""
    # Note that we're accessing a private attribute here.
    # This is a little unusual for a test, but suits our purposes
    # for this exercise, which is all about manipulating BSTs.
    assert bst.items_at_depth(2) == sorted(bst.items_at_depth(2))

def test_items_at_depth():
    bst = _list_to_bst([3, 1, 4])
    assert bst.items_at_depth(2) == [1, 4]
...
def test_levels_handmade():
    bst = _list_to_bst([3, 1, 4])
    assert bst.levels() == [(1, [3]), (2, [1, 4])]
