...
def test_to_example():
    ...
    assert a.to_nested_list() == ['A', ['B', ['E'], ['F']],
                                  ['C', ['G'], ['H', ['J']]],
                                  ['D', ['I']]]


def test_to_nested_list_line_except_lasr():
    """Test to_nested_list on a tree with a linear structure."""
    t5 = Tree(5, [])
    t5_a = Tree(6, [])
    t4 = Tree(4, [t5, t5_a])
    ...
    assert t1.to_nested_list() == [1, [2, [3, [4, [5], [6]]]]]


def test_to_nested_list_non_line():
    """Test to_nested_list on a tree with a linear structure."""
    t5 = Tree(5, [])
    t5_a = Tree(6, [])
    t4 = Tree(4, [t5, t5_a])
    ...
    assert t1.to_nested_list() == [1, [2, [3, [4, [5], [6]], [7]]], [15]]
...
def test_cluster():
    ...
    t2_a = Tree(15, [])
    t1 = Tree(1, [t2, t2_a])
    list = t1.to_nested_list()

    assert to_tree(list) == t1
