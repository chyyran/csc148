...
def _anagrams_helper(words: List[str], letter_count: Dict[str, int],
    ...
    for word in words:
        ...
        word_letter_count = _generate_letter_count(word)
        new_letter_count = {letter: count - word_letter_count[letter] for
                            (letter, count)
                            in letter_count.items()}
        word_anagrams = _anagrams_helper(words, new_letter_count, limit)
        anagrams_list.extend([(word + " " + anagram).strip() for anagram
                              in word_anagrams])
